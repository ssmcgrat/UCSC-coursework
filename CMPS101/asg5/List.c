...
int isEmpty(List L){
   if(L==NULL){
      printf("List Error: calling isEmpty() on a NULL List\n");
      exit(1);
   }
   return (L->length==0);
}
...
// getIndex()==length-1, cursor becomes undefined. If getIndex()==-1, cursor remains undefined.// This operation is equivalent to moveTo(getIndex()+1).
