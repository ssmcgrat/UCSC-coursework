...
class inode {
   ...
   public:
      ...
      void addDir(string str, inode_ptr parent);
      void mkFile(const string& filename);
      void printContents();
      void printRecursive(inode_ptr ptr);
      size_t size();
      // called on to put '.' and '..' entries into a newly created dir
      void insertIntoDir(inode_ptr parent, inode_ptr curr);
      string name;
      bool findChild(string child); 
      inode_ptr getChild(string child);
      inode_t get_type();
      void writeFile(const wordvec& words);
      void readFile() const;
      void deleteFile(const string& filename);
      void deleteFileRec(inode_ptr parent, 
...
class directory: public file_base {
   ...
   public:
      ...
      void removeRecursive(inode_ptr parent,
                           inode_ptr curr, const string& filename);
      void mkdir (const string& dirname, inode_ptr parent); 
      void mkfile (const string& filename); // inode& mkfile
      void printDirents();
      inode_ptr printRecursive(inode_ptr ptr);
      // creates the '.' and '.. entries in a newly created directory
      void insertIntoDir(inode_ptr parent, inode_ptr curr); 
      bool findChild(string child);
      inode_ptr getChild(string child);
};
